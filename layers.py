...
class GCNConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("x_gcnconv_output: ", x_gcnconv)
    ...
    #     x_first_mul = torch.mm(x.reshape(-1, c_in), self.weight).view(n_vertex, -1)
    #     x_second_mul = torch.mm(mat, x_first_mul).view(-1, self.c_out)
...
class GRDCNConv(nn.Module):
    def __init__(self, c_in, c_out, gcnconv_matrix, enable_bias):
        super(GRDCNConv, self).__init__()
        self.c_in = c_in
        self.c_out = c_out
        self.gcnconv_matrix = gcnconv_matrix
        self.enable_bias = enable_bias
        self.weight_reaction = nn.Parameter(torch.FloatTensor(c_in, c_out))
        self.weight_diffusion = nn.Parameter(torch.FloatTensor(c_in, c_out))

        self.weight_gate = nn.Parameter(torch.FloatTensor(c_in, c_out))
        # self.weight_gate_diffusion = nn.Parameter(torch.FloatTensor(1, c_out))

        if enable_bias == True:
            self.bias_reaction = nn.Parameter(torch.FloatTensor(c_out))
            self.bias_diffusion = nn.Parameter(torch.FloatTensor(c_out))
            self.bias_gate = nn.Parameter(torch.FloatTensor(c_out))
        else:
            self.register_parameter('bias', None)
        self.initialize_parameters()
    ...
    def initialize_parameters(self):
        init.xavier_uniform_(self.weight_reaction)
        init.xavier_uniform_(self.weight_diffusion)
        init.xavier_uniform_(self.weight_gate)

        if self.bias_reaction is not None:
            init.zeros_(self.bias_reaction)
            init.zeros_(self.bias_diffusion)
            init.zeros_(self.bias_gate)

    def forward(self, x):
        batch_size, c_in, T, n_vertex = x.shape

        x_first_mul_diff = torch.mm(x.reshape(-1, c_in), self.weight_diffusion).view(n_vertex, -1)
        x_second_mul_diff = torch.mm(self.gcnconv_matrix[0], x_first_mul_diff).view(-1, self.c_out)

        x_first_mul_reaction = torch.mm(x.reshape(-1, c_in), self.weight_reaction).view(n_vertex, -1)
        x_second_mul_reaction = torch.mm(self.gcnconv_matrix[1], x_first_mul_reaction).view(-1, self.c_out)

        x_gate = torch.mm(x.reshape(-1, c_in), self.weight_gate) + self.bias_gate
        x_gate = self.sigmoid(x_gate)

        if self.bias_reaction is not None:
            x_gcnconv = x_gate * (x_second_mul_diff + self.bias_diffusion) + (1-x_gate) * self.tanh(x_second_mul_reaction + self.bias_reaction)
        else:
            x_gcnconv = x_second_mul_diff + self.tanh(x_second_mul_diff)
...
#         self.weight_sum_reaction = nn.Parameter(torch.FloatTensor(c_in, c_out))
...
#         if self.bias_reaction is not None:
#             x_gcnconv = x_second_mul_diff + self.bias_diffusion + self.tanh(x_second_mul_reaction + self.bias_reaction)
#         else:
#             x_gcnconv = x_second_mul_diff + self.tanh(x_second_mul_diff)
...
class GraphConvLayer(nn.Module):
    def __init__(self, Ks, c_in, c_out, graph_conv_type, graph_conv_matrix):
        ...
        elif self.graph_conv_type == 'grdcnconv':
            self.gcnconv = GRDCNConv(c_out, c_out, graph_conv_matrix, self.enable_bias)
    ...
    def forward(self, x):
        ...
        # aligned_x_gc = self.align(x_gc)
        # x_gc_with_rc = torch.add(aligned_x_gc, x_gc_in)
        # aligned_x_gc = self.align(x_gc)
        # x_gc_out = aligned_x_gc
    ...
    #         x_gc = self.chebconv(x_gc_in)
    #     elif self.graph_conv_type == 'gcnconv':
    #         x_gc = self.gcnconv(x_gc_in, mat)
    #     elif self.graph_conv_type == 'grdcnconv':
    #         x_gc = self.gcnconv(x_gc_in)
    #     elif self.graph_conv_type == "reactiondiffusionconv":
    #         x_gc = self.gcnconv(x, torch.rand(x.shape[0]))
    #     # x_gc_with_rc = torch.add(x_gc.view(batch_size, self.c_out, T, n_vertex), x_gc_in)
